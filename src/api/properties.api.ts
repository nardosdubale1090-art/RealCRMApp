...
export const getSites = (): Promise<Site[]> => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const sitesWithoutChildren = mockSites.map(
        ({ buildings, ...site }) => site
      );
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (site && site.buildings) {
        ...
        const buildingsWithoutChildren = site.buildings.map(
...
export const getBuildingDetails = (
  id: string
): Promise<{ building: Building; site: Site } | undefined> => {
...
export const getUnitDetails = (
  id: string
): Promise<{ unit: Unit; building: Building; site: Site } | undefined> => {
...
export const createProperty = (
  type: "site" | "building" | "unit",
  data: any
) => {
  console.log(`API: Creating ${type}`, data);
  return new Promise((resolve) =>
    setTimeout(
      () => resolve({ id: `new_${type}_${Date.now()}`, ...data }),
      API_DELAY
    )
  );
};
...
export const updateProperty = (
  ...
  return new Promise((resolve) =>
    setTimeout(() => resolve({ id, ...data }), API_DELAY)
  );
};
