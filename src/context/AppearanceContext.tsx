...
export const ACCENT_PALETTES: Record<AccentColor, { label: string, primaryHex: string, vars: Record<string, string> }> = {
  indigo: {
    label: 'Indigo',
    primaryHex: '#6366f1',
    vars: {
      '--color-primary-hsl': '239 82% 66%', '--color-primary-hover-hsl': '239 92% 72%',
      '--color-chart-1': '#3b82f6', '--color-chart-2': '#14b8a6', '--color-chart-3': '#ef4444', '--color-chart-4': '#f97316', '--color-chart-5': '#8b5cf6',
    }
  },
  forest: {
    label: 'Forest',
    primaryHex: '#16a34a',
    vars: {
      '--color-primary-hsl': '145 74% 36%', '--color-primary-hover-hsl': '145 84% 42%',
      '--color-chart-1': '#22c55e', '--color-chart-2': '#84cc16', '--color-chart-3': '#facc15', '--color-chart-4': '#06b6d4', '--color-chart-5': '#a855f7',
    }
  },
  rose: {
    label: 'Rose',
    primaryHex: '#e11d48',
    vars: {
      '--color-primary-hsl': '347 87% 49%', '--color-primary-hover-hsl': '347 97% 55%',
      '--color-chart-1': '#f43f5e', '--color-chart-2': '#d946ef', '--color-chart-3': '#6366f1', '--color-chart-4': '#22d3ee', '--color-chart-5': '#f59e0b',
    }
  },
  sunrise: {
    label: 'Sunrise',
    primaryHex: '#f97316',
    vars: {
      '--color-primary-hsl': '25 95% 53%', '--color-primary-hover-hsl': '25 98% 58%',
      '--color-chart-1': '#ea580c', '--color-chart-2': '#eab308', '--color-chart-3': '#ef4444', '--color-chart-4': '#84cc16', '--color-chart-5': '#3b82f6',
    }
  },
  violet: {
    label: 'Violet',
    primaryHex: '#8b5cf6',
    vars: {
      '--color-primary-hsl': '258 90% 66%', '--color-primary-hover-hsl': '258 95% 72%',
      '--color-chart-1': '#a855f7', '--color-chart-2': '#ec4899', '--color-chart-3': '#22d3ee', '--color-chart-4': '#4ade80', '--color-chart-5': '#fde047',
    }
  },
  ocean: {
    label: 'Ocean',
    primaryHex: '#06b6d4',
    vars: {
      '--color-primary-hsl': '187 94% 43%', '--color-primary-hover-hsl': '187 98% 48%',
      '--color-chart-1': '#0891b2', '--color-chart-2': '#34d399', '--color-chart-3': '#a78bfa', '--color-chart-4': '#fb923c', '--color-chart-5': '#f472b6',
    }
  },
};
...
export const FONT_FAMILIES: Record<FontFamily, { name: string, css: string }> = {
    ...
    'playfair-display': { name: 'Playfair Display', css: "'Playfair Display', serif" },
...
const BASE_THEME_OPTIONS: BaseTheme[] = ['system', 'light', 'dark-pro', 'midnight'];
const ACCENT_COLOR_OPTIONS: AccentColor[] = Object.keys(ACCENT_PALETTES) as AccentColor[];
const LAYOUT_OPTIONS: Layout[] = ['vertical', 'horizontal'];
const MOBILE_LAYOUT_OPTIONS: MobileLayout[] = ['bottom', 'sidebar'];
const FONT_FAMILY_OPTIONS: FontFamily[] = Object.keys(FONT_FAMILIES) as FontFamily[];
...
const getStoredValue = <T,>(key: string, options: T[], defaultValue: T): T => {
    if (typeof window !== 'undefined') {
        ...
        if (storedValue) {
            ...
            const parsed = (typeof defaultValue === 'number' ? parseInt(storedValue, 10) : storedValue) as T;
...
const getInitialNavLinks = (): NavLinkItem[] => {
    ...
    try {
        ...
        const defaultLinksMap = new Map(DEFAULT_NAV_LINKS.map(link => [link.id, link]));
...
export const AppearanceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [baseTheme, setBaseThemeState] = useState<BaseTheme>(() => getStoredValue('re-crm-base-theme', BASE_THEME_OPTIONS, 'system'));
  const [accentColor, setAccentColorState] = useState<AccentColor>(() => getStoredValue('re-crm-accent', ACCENT_COLOR_OPTIONS, 'indigo'));
  const [layout, setLayoutState] = useState<Layout>(() => getStoredValue('re-crm-layout', LAYOUT_OPTIONS, 'vertical'));
  const [mobileLayout, setMobileLayoutState] = useState<MobileLayout>(() => getStoredValue('re-crm-mobile-layout', MOBILE_LAYOUT_OPTIONS, 'bottom'));
  const [fontFamily, setFontFamilyState] = useState<FontFamily>(() => getStoredValue('re-crm-font-family', FONT_FAMILY_OPTIONS, 'inter'));
  const [baseFontSize, setBaseFontSizeState] = useState<BaseFontSize>(() => getStoredValue('re-crm-font-size', BASE_FONT_SIZE_OPTIONS, 16));
  const [isSidebarCollapsed, setIsSidebarCollapsedState] = useState<boolean>(getInitialSidebarCollapsed);
  const [navLinks, setNavLinksState] = useState<NavLinkItem[]>(getInitialNavLinks);
  ...
  const value = useMemo(() => ({ 
      baseTheme, accentColor, layout, mobileLayout, fontFamily, baseFontSize, isSidebarCollapsed, navLinks, 
      setBaseTheme, setAccentColor, setLayout, setMobileLayout, setFontFamily, setBaseFontSize, setIsSidebarCollapsed, setNavLinks,
      resetAppearance
  }), [baseTheme, accentColor, layout, mobileLayout, fontFamily, baseFontSize, isSidebarCollapsed, navLinks, resetAppearance, setBaseTheme, setAccentColor, setLayout, setMobileLayout, setFontFamily, setBaseFontSize, setIsSidebarCollapsed, setNavLinks]);
