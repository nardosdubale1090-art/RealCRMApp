...
import { UsersIcon, BanknotesIcon, CalendarDaysIcon, BriefcaseIcon } from '../shared/Icons';

const iconMap: { [key in RecentActivity['type']]: React.ComponentType<{className?: string}> } = {
...
const RecentActivities: React.FC = () => {
    return (
        <div className="bg-card p-6 rounded-lg shadow-lg h-full">
            <h3 className="text-xl font-semibold mb-4 text-text-primary">Recent Activities</h3>
                        ...
                        <li key={activity.id} className="flex items-start space-x-4">
                            ...
                            <div className="flex-1">
                                <p className="text-sm text-text-primary">
                                    <span className="font-semibold">{activity.user}</span> {activity.type === 'New Client' || activity.type === 'Site Visit' ? '' : 'updated a'} {activity.description}
                                </p>
